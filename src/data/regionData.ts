/**
 * 시·도별 구(시·군) 목록 — 희망 지역 설정용
 * 저장 형식: "시도 구" (예: "서울 강남구", "경기 수원시")
 */
...
export const REGION_SIDO_GU: RegionSido[] = [
  {
    sido: '서울',
    guList: [
  ...
  {
    sido: '대구',
    guList: ['남구', '달서구', '달성군', '동구', '북구', '서구', '수성구', '중구'],
  },
  {
    sido: '인천',
    guList: [
...
/** 저장된 값에서 시도만 추출 (예: "서울 강남구" -> "서울") */
export function getSidoFromValue(value: string): string {
  const space = value.indexOf(' ');
  return space > 0 ? value.slice(0, space) : value;
}

/** 저장된 값에서 구만 추출 (예: "서울 강남구" -> "강남구") */
export function getGuFromValue(value: string): string {
  const space = value.indexOf(' ');
  return space > 0 ? value.slice(space + 1) : value;
}

/** 시도 + 구로 저장용 문자열 생성 */
export function toRegionValue(sido: string, gu: string): string {
  return `${sido} ${gu}`;
}
